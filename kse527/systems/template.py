...
from kse527.control.controllers import dummy_zero_controller
...
class ControlledSystemTemplate(nn.Module):
    ...
    """
    def __init__(self,
                 u=dummy_zero_controller, 
                 solver='euler', 
                 retain_u=False,
                 swap_batch_time=True,
                 **odeint_kwargs):
        super().__init__()
        self.u = u 
        self.solver = solver
        ...
        self.swap_batch_time = swap_batch_time
    ...
    def forward(self, x0, t_span):
        ...
        if self.retain_u:
            ...
            for i in range(len(t_span)-1):
                ...
                xt = odeint(self.dynamics, xt, diff_span, solver=self.solver, **self.odeint_kwargs)[1][-1]
                x.append(xt[None])
            traj = torch.cat(x)
        else:
